...
void AP_Mount::update(uint8_t mount_compid,  AP_SerialManager& serial_manager)
{
    ...
    static AP_HAL::UARTDriver *uart = serial_manager.find_serial(AP_SerialManager::SerialProtocol_MAVLink,2);
    static bool begin_gmb_uart;
    static uint32_t baud = serial_manager.find_baudrate(AP_SerialManager::SerialProtocol_MAVLink,2);
    for (uint8_t instance=0; instance<AP_MOUNT_MAX_INSTANCES; instance++) {
