...
void NavEKF3_core::writeOptFlowMeas(uint8_t &rawFlowQuality, Vector2f &rawFlowRates, Vector2f &rawGyroRates, uint32_t &msecFlowMeas)
{
        ...
        if (magTimeout && (maxCount > 1) && !onGround && imuSampleTime_ms - ekfStartTime_ms > 30000) {
            ...
            for (uint8_t i=1; i<maxCount; i++) {
                ...
                if (_ahrs->get_compass()->use_for_yaw(tempIndex) && tempIndex != magSelectIndex) {
                    ...
                    hal.console->printf("EKF3 IMU%u switching to compass %u\n",(unsigned)imu_index,magSelectIndex);
