...
void NavEKF3_core::controlMagYawReset()
{
    ...
    if (magYawResetRequest || magStateResetRequest) {
        ...
        if (magYawResetRequest) {
            ...
            if (!yawAlignComplete) {
                hal.console->printf("EKF3 IMU%u initial yaw alignment complete\n",(unsigned)imu_index);
            }

            // send in-flight yaw alignment status to console
            if (finalResetRequest) {
                hal.console->printf("EKF3 IMU%u in-flight yaw alignment complete\n",(unsigned)imu_index);
            } else if (interimResetRequest) {
                hal.console->printf("EKF3 IMU%u ground mag anomaly, yaw re-aligned\n",(unsigned)imu_index);
            }
