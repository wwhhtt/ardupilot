...
void NavEKF3::writeOptFlowMeas(uint8_t &rawFlowQuality, Vector2f &rawFlowRates, Vector2f &rawGyroRates, uint32_t &msecFlowMeas)
{
    if (core) {
        for (uint8_t i=0; i<num_cores; i++) {
...
uint32_t NavEKF3::getLastYawResetAngle(float &yawAng) const
{
    if (!core) {
        return 0;
    }
...
uint32_t NavEKF3::getLastPosNorthEastReset(Vector2f &pos) const
{
    if (!core) {
        return 0;
    }
